...
class RigTestData(Document):
    """Child Rig Data records shown inside a Rig Test detail page."""
    rig_id = ObjectIdField(required=True)
    type_of_rig_test = StringField(required=True)
    software_version = StringField(required=True)
    pil_no = StringField(required=True)
    file_no = StringField(required=True)
    created_date = DateTimeField(default=datetime.utcnow)
